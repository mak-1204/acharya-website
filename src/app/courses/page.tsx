...
export default function CoursesPage() {
  ...
  const handleFilterChange = (id: string) => {
    setActiveCourse(id);
    // Scroll to top of content area on mobile when filter changes
  ...
  useEffect(() => {
    ...
    if (hash && COURSES_DATA.find(c => c.id === hash)) {
  ...
  const displayCourses = activeCourse === 'all' 
    ? COURSES_DATA 
    : COURSES_DATA.filter(c => c.id === activeCourse);
          ...
          <div className="lg:w-3/4 space-y-8 md:space-y-12">
             {displayCourses.length > 0 ? (
               displayCourses.map((course) => (
                 <section key={course.id} id={course.id} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
